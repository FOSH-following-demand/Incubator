...
1.	Gather all necessary equipment, electronics and sensors listed in the BOM, keep your soldering kit and Digital multimeter ready to check connections and normal operation after soldering. 
2.	Using a breadboard, Carefully connect all the sensors and other modules to the Arduino Uno microprocessor following the equipment’s electronic circuit diagrams. Most connections are made through jumpers, relay module, light bulb and power supply module are connected with standard IEC Electrical power cables.
3.	Using a multimeter make sure all connections are ok and that the current flows normally.
4.	Connect the Arduino UNO board to a computer with the latest version of Arduino IDE installed using a USD-Arduino Uno cable.
5.	Open the Incubator.ino software and click on verify to make sure that the entire necessary libraries are installed.
...
8.	Use the 2D CAD files: Incubator_Enclosure, to laser-cut plywood or acrylic to make the electronic enclosure of the incubator. 4-5mm colored acrylic is recommended. Use acrylic glue to assemble cut pieces and first mount the base of the box. 
...
15.	Carefully fix the electronic control box on top of the cooler box Cover using double-sided tape.
...
18.	After checking that everything is still operational, place the acrylic cover on top of the electronic box (don’t use acrylic glue to seal the box) the cover should be easily removable for maintenance.
